...
if [ $stage -le 9 ]; then
  for test in dev_clean test_clean dev_other test_other; do
    ...
    "ark:ivector-subtract-global-mean exp/ivectors_train/mean.vec scp:exp/ivectors_${test}/ivector.scp ark:- | transform-vec exp/ivectors_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-subtract-global-mean exp/ivectors_train/mean.vec scp:exp/ivectors_${test}/ivector.scp ark:- | transform-vec exp/ivectors_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
