...
steps/align_fmllr.sh --nj 16 --cmd "$train_cmd" \
  data/train.1k data/lang exp/tri3a exp/tri3a_ali || exit 1;

steps/train_lda_mllt.sh --cmd "$train_cmd" --realign-iters "" \
  1000 10000 data/train.1k data/lang exp/tri3a_ali exp/tri4a_seg || exit 1;

steps/align_fmllr.sh --nj 16 --cmd "$train_cmd" \
  data/train data/lang exp/tri3a exp/tri3a_ali || exit 1;

# Make the phone decoding-graph.
steps/make_phone_graph.sh data/lang exp/tri3a_ali exp/tri4a_seg || exit 1;
...
steps/decode_nolats.sh --write-words false --write-alignments true \
  ...
  exp/tri4a_seg/phone_graph data_reseg/train_orig exp/tri4a_seg/decode_train_orig

steps/decode_nolats.sh --write-words false --write-alignments true \
   --cmd "$decode_cmd" --nj 10 --beam 7.0 --max-active 1000 \
  exp/tri4a_seg/phone_graph data_reseg/eval2000_orig exp/tri4a_seg/decode_eval2000_orig


# Here: resegment.
# Note: it would be perfectly possible to use exp/tri3a_ali_train here instead
# of exp/tri4a_seg/decode_train_orig.  In this case we'd be relying on the transcripts.
...
steps/resegment_data.sh --cmd "$train_cmd" data_reseg/train_orig data/lang \
  exp/tri4a_seg/decode_train_orig data_reseg/train exp/tri4a_resegment_train

#steps/resegment_data.sh --cmd "$train_cmd" data_reseg/eval2000_orig data/lang \
#  exp/tri4a_seg/decode_eval2000_orig data_reseg/eval2000 exp/tri4a_resegment_eval2000

# We need all the training data to be aligned (not just "train_nodup"), in order
# to get the resegmented "text".
steps/align_fmllr.sh --nj 16 --cmd "$train_cmd" \
  data/train data/lang exp/tri3a exp/tri3a_ali_train || exit 1;

# Get the file data_reseg/train/text
steps/resegment_text.sh --cmd "$train_cmd" data/train data/lang \
  exp/tri3a_ali_train data_reseg/train exp/tri4a_resegment_train
...
# Note: we'll be comparing tri4a, which was trained on train_nodup, with tri4c_reseg, which
...
steps/align_fmllr.sh --nj 40 --cmd "$train_cmd" \
  data_reseg/train data/lang exp/tri3a exp/tri3a_ali_reseg || exit 1;

steps/train_sat.sh  --cmd "$train_cmd" \
  11500 200000 data_reseg/train data/lang exp/tri3a_ali_reseg exp/tri4a_reseg || exit 1;


for lm_suffix in tg fsh_tgpr; do
  (
    graph_dir=exp/tri4a_reseg/graph_sw1_${lm_suffix}
    $train_cmd $graph_dir/mkgraph.log \
      utils/mkgraph.sh data/lang_sw1_${lm_suffix} exp/tri4a_reseg $graph_dir
    steps/decode_fmllr.sh --nj 16 --cmd "$decode_cmd" --config conf/decode.config \
       $graph_dir data_reseg/eval2000 exp/tri4a_reseg/decode_eval2000_sw1_${lm_suffix}
  ) &
done
...
#  exp/tri4a_seg/decode_eval2000_orig data_reseg/eval2000_with_noise exp/tri4a_resegment_eval2000_with_noise
#
