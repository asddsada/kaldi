...
if [ $stage -le 1 ]; then
  ...
  #utils/subset_data_dir.sh data/train_hires 3024 data/train_hires_300
  # want the 100k subset to exactly match train_100k, since we'll use its alignments.
  #awk '{print $1}' data/train/utt2spk > uttlist
...
if [ $stage -le 2 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    ...
    1000 3000 data/train_hires data/lang exp/tri4a exp/nnet2_online/tri5a
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 4 --num-frames 4000 \
    data/train_hires 128 exp/nnet2_online/tri5a exp/nnet2_online/diag_ubm
fi
